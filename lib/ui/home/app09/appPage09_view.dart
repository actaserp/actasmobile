...
class _AppPage09viewState extends State<AppPage09view> {
  ...
  Future downmb() async {
    _dbnm = await  SessionManager().get("dbnm");
    var uritxt = CLOUD_URL + '/mobile/download?dbnm=$_dbnm&flag=${widget.EData.eflag}&boardIdx=${widget.EData.eseq}&idx=${_idxData.toString().substring(1,3)}&inputdate=${_inData.toString().substring(1,11).replaceAll('-', '')}&svn=${_SaNData.toString().substring(1).replaceAll(']', '')}&ori=${_ATCData.toString().substring(1).replaceAll(']', '')}';
    var encoded = Uri.encodeFull(uritxt);
    Uri uri = Uri.parse(encoded);
    print('@@@@@@@@@@@@@@@데이터 테스트@@@@@@@@@@@@@@@@@@@@@');
                              ...
                              children: [
                                InteractiveViewer(
                                  ...
                                  child: ClipRRect(
                                      ...
                                      child: buildCacheNetworkImage(width: boxImageSize, height: boxImageSize, url: "$CLOUD_URL" + "/appx/download?actidxz=${_idxData[index]}&actboardz=${_seqData[index]}&actflagz=EE")
                                  ),
                                ),
                                Text('${_ATCData[index]}',
                                  style: TextStyle(
