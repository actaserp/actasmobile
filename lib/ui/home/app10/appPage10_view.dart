...
class AppPage10view extends StatefulWidget {
  final DmanualList_model DData;
  const AppPage10view({Key? key, required this.DData}) : super(key: key);

  @override
  _AppPage10viewState createState() => _AppPage10viewState();
}

class _AppPage10viewState extends State<AppPage10view> {
  ...
  Future attachfiles()async {
    ...
    final response = await http.post(
      ...
      body: <String, String> {
        ...
        'dbnm': _dbnm,
        'flag':  'DD',
        'boardIdx' : widget.DData.dseq.toString(),
      },
    );
  ...
  Future downmb() async {
    _dbnm = await  SessionManager().get("dbnm");
    var uritxt = CLOUD_URL + '/mobile/download?dbnm=$_dbnm&flag=DD&boardIdx=${widget.DData.dseq}&idx=${_idxData.toString().replaceAll('[', '').replaceAll(']', '')}&inputdate=${_inData.toString().substring(1,11).replaceAll('-', '')}&svn=${_SaNData.toString().substring(1).replaceAll(']', '')}&ori=${_ATCData.toString().substring(1).replaceAll(']', '')}';
    var encoded = Uri.encodeFull(uritxt);
    Uri uri = Uri.parse(encoded);
    print('@@@@@@@@@@@@@@@데이터 테스트@@@@@@@@@@@@@@@@@@@@@');
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  child: Text('제목 : ${widget.DData.dsubject}', style: TextStyle(
                      fontSize:16, fontWeight: FontWeight.bold, color: SOFT_BLUE
                  )),
                ),
                Container(
                  margin: EdgeInsets.only(top: 8),
                  child: Text('분류 : ${widget.DData.cnam}', style: TextStyle(
                ...
                Container(
                  child: Row(
                    children: [
                      Text('작성자 : ${widget.DData.dpernm}',style: TextStyle(
                          fontSize: 13, fontWeight: FontWeight.bold
                      ))
                    ],
                  ),
                ...
                Container(
                  margin: EdgeInsets.only(top: 8),
                  child: Text('${widget.DData.dmemo}', style: TextStyle(
                ...
                Container(
                  ...
                  child: Text('첨부파일리스트(${widget.DData.attcnt}개)', style: TextStyle(
                              ...
                              children: [
                                InteractiveViewer(
                                  ...
                                  child: ClipRRect(
                                      ...
                                      child: buildCacheNetworkImage(width: boxImageSize, height: boxImageSize, url: "$CLOUD_URL" + "/appx/download?actidxz=${_idxData[index]}&actboardz=${_seqData[index]}&actflagz=DD")
                                  ),
                                ),
                                Text('${_ATCData[index]}',
                                  style: TextStyle(
