...
import 'package:actasm/model/app02/mnoticeList_model.dart';
...
class AppPager14Detail extends StatefulWidget {
  final mnoticeList_model mnoticeData;
  const AppPager14Detail({Key? key, required this.mnoticeData}) : super(key: key);

  // final MhmanualList_model mhData;
  // const AppPage03Detail({Key? key, required this.mhData}) : super(key: key);

  @override
  _AppPager14DetailState createState() => _AppPager14DetailState();

}

class _AppPager14DetailState extends State<AppPager14Detail> {
  ...
  @override
  void setData(){
    _attachidx = widget.mnoticeData.nseq;
    _etfnsubject = TextEditingController(text: widget.mnoticeData.nsubject);
    _etfmemo     = TextEditingController(text: widget.mnoticeData.nmemo);
  }

  @override
  Future comment()async {
    _dbnm = await  SessionManager().get("dbnm");
    var uritxt = CLOUD_URL + '/apppgymobile/attachNOTICE';
    ...
    final response = await http.post(
      ...
      body: <String, String> {
        ...
        'dbnm': _dbnm,
        'nflag':  "NN",
        'nseq' : widget.mnoticeData.nseq.toString(),
      },
    );
  ...
  Future<bool> update_data() async {
    ...
    if(_selectedValue == "" || _selectedValue == null || _selectedValue == ''){
      _selectedValue = widget.mnoticeData.ngourpcd;
    }

    var uritxt = CLOUD_URL + '/apppgymobile/mnoticeupdate';
    var encoded = Uri.encodeFull(uritxt);
    Uri uri = Uri.parse(encoded);
    // try {
    final response = await http.post(
      ...
      body: <String, String>{
        'nseq'    :  widget.mnoticeData.nseq,
        'nsubject': _etfnsubject.text,
        'nmemo'    : _etfmemo.text,
        'ngourpcd' : _selectedValue,
        'nflag'    : _shared,
        ...
        children: [
          Container(
            ...
            child: Row(
              ...
              children: [
                Text(
                    'No.${widget.mnoticeData.nseq}', style: TextStyle(
                    fontSize: 16, fontWeight: FontWeight.w700, color: CHARCOAL
                )
                ),
                Spacer(),
                Text(
                  '등록일자: ${widget.mnoticeData.ninputdate}', style: TextStyle(
                    fontSize: 16, fontWeight: FontWeight.w700, color: CHARCOAL
                ),
                  ...
                  child: Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        /* value: "aa",*/
                        hint: Text(widget.mnoticeData.cnam,  style: TextStyle(color: Colors.white)),
                        ...
                        onTap: () async {
                          ...
                          try{
                            await FlutterDownloader.enqueue(
                              url: "$CLOUD_URL" + "/nappx/download?actidxz=${_idxData[index]}&actboardz=${_seqData[index]}&actflagz=NN",
                              savedDir: '$dir',
                            ...
                            print("error :::: $e");
                            print("idx :::: $_idxData seq :::: $_seqData" + " url 시작 ::: $CLOUD_URL + /nappx/download?actidxz=?${_idxData[index]}&actboardz=${_seqData[index]}&actflagz=MF");
                          }
                        },
                          ...
                          child:  Column(
                            children: [
                              ClipRRect(
                                  ...
                                  child: buildCacheNetworkImage(width: 320, height: 200, url: "$CLOUD_URL" + "/nappx/download?actidxz=${_idxData[index]}&actboardz=${_seqData[index]}&actflagz=NN")
                              ),
